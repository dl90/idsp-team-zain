...
class Scene_1 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio('danger_audio', 'bgm/Meme_action.mp3');                                      //
  ...
  create() {
    ...
    gameState.emitter.on('danger_bgm_play', () => {
      if (this.danger_bgm_toggle) {
        sceneBGM.pause();
        dangerBGM.play();
        this.danger_bgm_toggle = false
      }
    }, this);
    ...
    // trees
    level_1.firTree.forEach(obj => { walls.create(obj.x * 32, obj.y * 32, 'fir_tree').setSize(12, 10).setOffset(25, 54).setDepth(2) });
    level_1.birchTree.forEach(obj => { walls.create(obj.x * 32, obj.y * 32, 'birch_tree').setSize(16, 12).setOffset(26, 46).setDepth(2) });
    ...
    this.enemies.getChildren().forEach(function (gameObj) {
      ...
      // overlap
      this.physics.add.overlap(gameObj, gameState.player, reduceHealth.bind(this));
