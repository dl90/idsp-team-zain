...
class Scene_2 extends Phaser.Scene {
  ...
  create() {
    ...
    this.camera = this.cameras.main.setBounds(0, 0, this.scene_2_settings.worldWidth, this.scene_2_settings.worldHeight);
    this.physics.world.setBounds(0, 0, this.scene_2_settings.worldWidth, this.scene_2_settings.worldHeight);

    // level title
    this.levelText = this.add.text(
      this.scene_2_settings.canvasWidth / 2,
      this.scene_2_settings.canvasHeight / 2,
      'Level 2',
      { fontSize: 30, color: '#000000' }
    ).setOrigin(0.5).setScrollFactor(0).setDepth(this.scene_2_settings.messageDepth);

    // control (cursors need to be recreated per scene)
    gameState.cursors = this.input.keyboard.createCursorKeys();
    gameState.player = this.physics.add.sprite(
      this.scene_2_settings.playerSpawnPosition[0] * 32,
      this.scene_2_settings.playerSpawnPosition[1] * 32,
      'f_dog').setSize(30, 30).setDepth(this.scene_2_settings.playerSpriteDepth).setOrigin(0);
    gameState.player.setCollideWorldBounds(true).setBounce(1);
    gameState.player.noStop = true;

    // healthBar
    gameState.healthBar = this.add.sprite(40, 20, 'health_100').setScrollFactor(0).setDepth(this.scene_2_settings.healthBarDepth);
    this.scene_2_settings.debug ? this.healthVal = 100 : this.healthVal = this.playerHealth; // gets health from passed value
    ...
    // time tracker
    this.levelTime = this.scene_2_settings.levelTime;
    this.add.rectangle(this.scene_2_settings.canvasWidth / 2 + 70, 10, 260, 15)
      .setFillStyle(0xffffff, 0.5).setScrollFactor(0).setDepth(this.scene_2_settings.scoreTimerBackgroundDepth);
    this.timeText = this.add.text(
      this.scene_2_settings.canvasWidth / 2, 10,
    ...
    this.scoreText = this.add.text(
      this.scene_2_settings.canvasWidth / 2 + 150, 10,
      `Score: ${this.score}`,
      { fontSize: 12, color: '#000000' }
    ).setOrigin(0.5).setScrollFactor(0).setDepth(this.scene_2_settings.scoreTextDepth);

    // scene transition
    this.girl = this.physics.add.sprite(this.scene_2_settings.familySpawnPosition[0] * 32, this.scene_2_settings.familySpawnPosition[1] * 32, 'girl').setOrigin(0);
    ...
    map.createStaticLayer('background', [tileSet], 0, 0).setDepth(this.scene_2_settings.backgroundDepth);
    const collider = map.createStaticLayer('collider', [tileSet], 0, 0).setDepth(this.scene_2_settings.wallSpriteDepth);
    ...
    facePhysicsGroup.getChildren().forEach(gameObj => {
      ...
      gameObj.setDepth(this.scene_2_settings.faceSpriteDepth);
    ...
    const audioButton = this.add.sprite(
      this.scene_2_settings.canvasWidth - 20,
      this.scene_2_settings.canvasHeight - 20,
      'audio_button_on').setScale(0.5).setScrollFactor(0).setDepth(this.scene_2_settings.buttonDepth).setInteractive();
    ...
    gameState.emitter.once('death_bgm', () => {
      ...
      this.levelText = this.add.text(
        this.scene_2_settings.canvasWidth / 2,
        this.scene_2_settings.canvasHeight / 2 - 50,
        'Game Over',
        { fontSize: 30, color: 'white' }).setScrollFactor(0).setOrigin(0.5).setDepth(this.scene_2_settings.messageDepth);
      this.tweens.add({
        targets: this.levelText,
        alpha: 0,
        duration: 2000,
      ...
      const mask = this.add.rectangle(
        ...
        this.camera.displayHeight + 20).setFillStyle(0x000000, 0.2).setScrollFactor(0).setDepth(this.scene_2_settings.deathBackgroundMaskDepth);
      this.tweens.add({
        targets: mask,
        duration: 4000,
        fillAlpha: 0.9
      ...
      const unmask = this.add.circle(
        ...
        10
      ).setFillStyle(0xffffff, 0).setScrollFactor(0).setDepth(this.scene_2_settings.deathBackgroundUnmaskDepth);
      this.tweens.add({
        targets: unmask,
        duration: 4000,
        fillAlpha: 0.9
    ...
    this.backButton = this.add.sprite(
      this.scene_2_settings.canvasWidth / 2,
      this.scene_2_settings.canvasHeight / 2 + 50,
      'back_button').setDepth(this.scene_2_settings.buttonDepth).setVisible(false).setOrigin(0.5).setInteractive().setScrollFactor(0);
  ...
  animate() {
    this.anims.create({
      ...
      frameRate: Math.round(this.scene_2_settings.moveSpeed / 15),
      repeat: -1
    });
    this.anims.create({
      key: 'b_move',
  ...
  update() {
    // health check
    if (this.healthVal > 0) {
      this.healthVal = gameFunctions.control(gameState, this.scene_2_settings, this.healthVal);
