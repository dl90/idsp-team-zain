...
class Scene_13 extends Phaser.Scene {
  ...
  create() {
    ...
    this.camera = this.cameras.main.setBounds(0, 0, this.scene_13_settings.worldWidth, this.scene_13_settings.worldHeight);
    this.physics.world.setBounds(0, 0, this.scene_13_settings.worldWidth, this.scene_13_settings.worldHeight);

    // level title
    this.levelText = this.add.text(
      this.scene_13_settings.canvasWidth / 2,
      this.scene_13_settings.canvasHeight / 2,
      'Level 13',
      { fontSize: 30, color: '#000000' }
    ).setOrigin(0.5).setScrollFactor(0).setDepth(this.scene_13_settings.messageDepth);

    // control (cursors need to be recreated per scene)
    gameState.cursors = this.input.keyboard.createCursorKeys();
    gameState.player = this.physics.add.sprite(
      this.scene_13_settings.playerSpawnPosition[0] * 32,
      this.scene_13_settings.playerSpawnPosition[1] * 32,
      'f_dog').setSize(30, 30).setDepth(this.scene_13_settings.playerSpriteDepth).setOrigin(0);
    gameState.player.setCollideWorldBounds(true).setBounce(1);

    // healthBar
    gameState.healthBar = this.add.sprite(40, 20, 'health_100').setScrollFactor(0).setDepth(this.scene_13_settings.healthBarDepth);
    this.scene_13_settings.debug ? this.healthVal = 1000 : this.healthVal = this.playerHealth; // gets health from passed value
    ...
    // time tracker
    this.levelTime = this.scene_13_settings.levelTime;
    this.add.rectangle(this.scene_13_settings.canvasWidth / 2 + 70, 10, 260, 15)
      .setFillStyle(0xffffff, 0.5).setScrollFactor(0).setDepth(this.scene_13_settings.scoreTimerBackgroundDepth);
    this.timeText = this.add.text(
      this.scene_13_settings.canvasWidth / 2, 10,
    ...
    this.scoreText = this.add.text(
      this.scene_13_settings.canvasWidth / 2 + 150, 10,
      `Score: ${this.score}`,
      { fontSize: 12, color: '#000000' }
    ).setOrigin(0.5).setScrollFactor(0).setDepth(this.scene_13_settings.scoreTextDepth);

    // scene transition
    this.boy = this.physics.add.sprite(
      this.scene_13_settings.familySpawnPosition[0] * 32,
      this.scene_13_settings.familySpawnPosition[1] * 32,
      'boy'
    ).setOrigin(0).setDepth(this.scene_13_settings.playerSpriteDepth);
    ...
    // background
    map.createStaticLayer('background', [playground], 0, 0).setDepth(this.scene_13_settings.backgroundDepth);

    // box
    const box = map.createStaticLayer('box', [playground], 0, 0).setDepth(this.scene_13_settings.wallSpriteDepth);
    ...
    // tree
    const tree = map.createStaticLayer('tree', [playground], 0, 0).setDepth(this.scene_13_settings.treeSpriteDepth);
    ...
    // plaything
    const plaything = map.createStaticLayer('plaything', [playground], 0, 0).setDepth(this.scene_13_settings.wallSpriteDepth);
    ...
    // flower (pass through)
    const flower = map.createStaticLayer('flower', [playground], 0, 0).setDepth(this.scene_13_settings.wallSpriteDepth);
    ...
    this.physics.add.collider(ballPhysicsGroup, gameState.player, () => {
      if (ballCollider) {
        ...
        const message = this.add.text(
          this.scene_13_settings.canvasWidth / 2,
          this.scene_13_settings.canvasHeight - 30,
          str,
    ...
    coin.forEachTile(tile => {
      ...
      if (tile.properties.coin) {
        coinPhysicsGroup.create(tileWorldPos.x + 16, tileWorldPos.y + 16, 'coin').setCircle(30 / 2, 1, 1).setDepth(this.scene_13_settings.itemSpriteDepth);
      }
    });
    coinPhysicsGroup.getChildren().forEach(gameObj => {
      this.physics.add.overlap(gameState.player, gameObj, () => {
        this.bonusScore += this.scene_13_settings.coinScoreBonus;
    ...
    bone.forEachTile(tile => {
      ...
      if (tile.properties.bone) {
        bonePhysicsGroup.create(tileWorldPos.x + 16, tileWorldPos.y + 16, 'bone').setCircle(5, 10, 10).setDepth(this.scene_13_settings.itemSpriteDepth);
      }
    });
    bonePhysicsGroup.getChildren().forEach(gameObj => {
      this.physics.add.overlap(gameState.player, gameObj, () => {
        (this.healthVal + this.scene_13_settings.boneHealthRegen) > 100 ? this.healthVal = 100 : this.healthVal += this.scene_13_settings.boneHealthRegen;
    ...
    this.scene_13_settings.enemy.forEach(function (obj) {
    ...
    enemyPhysicsGroup.getChildren().forEach(function (gameObj) {
      if (gameObj.getData("tweenX") !== 0) {
        this.tweens.add({
          ...
          ease: 'Linear',
          duration: Math.abs(gameObj.getData("tweenX")) * this.scene_13_settings.enemyTweenDurationMultiplier,
          repeat: -1,
          yoyo: true,
          loopDelay: this.scene_13_settings.enemyTweenLoopDelay
        });
        ...
        this.tweens.add({
          ...
          ease: 'Linear',
          duration: Math.abs(gameObj.getData("tweenY")) * this.scene_13_settings.enemyTweenDurationMultiplier,
          repeat: -1,
          yoyo: true,
          loopDelay: this.scene_13_settings.enemyTweenLoopDelay
        });
    ...
    const audioButton = this.add.sprite(
      this.scene_13_settings.canvasWidth - 20,
      this.scene_13_settings.canvasHeight - 20,
      'audio_button_on').setScale(0.5).setScrollFactor(0).setDepth(this.scene_13_settings.buttonDepth).setInteractive();
    ...
    gameState.emitter.once('death_bgm', () => {
      ...
      this.levelText = this.add.text(
        this.scene_13_settings.canvasWidth / 2,
        this.scene_13_settings.canvasHeight / 2 - 50,
        'Game Over',
        { fontSize: 30, color: 'white' }).setScrollFactor(0).setOrigin(0.5).setDepth(this.scene_13_settings.messageDepth);
      this.tweens.add({
        targets: this.levelText,
        alpha: 0,
        duration: 2000,
      ...
      const mask = this.add.rectangle(
        ...
        this.camera.displayHeight + 20).setFillStyle(0x000000, 0.2).setScrollFactor(0).setDepth(this.scene_13_settings.deathBackgroundMaskDepth);
      this.tweens.add({
        targets: mask,
        duration: 4000,
        fillAlpha: 0.9
      ...
      const unmask = this.add.circle(
        ...
        10
      ).setFillStyle(0xffffff, 0).setScrollFactor(0).setDepth(this.scene_13_settings.deathBackgroundUnmaskDepth);
      this.tweens.add({
        targets: unmask,
        duration: 4000,
        fillAlpha: 0.9
    ...
    this.backButton = this.add.sprite(
      this.scene_13_settings.canvasWidth / 2,
      this.scene_13_settings.canvasHeight / 2 + 50,
      'back_button').setDepth(this.scene_13_settings.buttonDepth).setVisible(false).setOrigin(0.5).setInteractive().setScrollFactor(0);
  ...
  animate() {
    this.anims.create({
      ...
      frameRate: Math.round(this.scene_13_settings.moveSpeed / 15),
      repeat: -1
    });
    this.anims.create({
      key: 'b_move',
    ...
    this.anims.create({
      ...
      frameRate: Math.round(this.scene_13_settings.moveSpeed / 40),
      repeat: -1
    });
  }
  ...
  update() {
    if (gameState.player) {
      this.physics.overlap(gameState.player, this.flowerPhysicsGroup) ?
        ...
        gameState.player.setDamping(false).setDrag(1).setMaxVelocity(this.scene_13_settings.moveSpeed);
    }
    ...
    this.tweens.getAllTweens().forEach(tween => {
      ...
      if (gameObj.type === "Sprite") {
        ...
        if (this.physics.overlap(gameState.player, gameObj)) {
          ...
          if (this.healthVal > 0) {
            this.healthVal -= this.scene_13_settings.enemyHealthReduction;
            this.dangerState = true;
          } else {
            this.HealthVal = -1;
          }
        } else if (distanceCalc(gameState.player, gameObj) < this.scene_13_settings.enemyChaseDistance) {
          tween.pause();
          this.physics.moveToObject(gameObj, gameState.player, this.scene_13_settings.enemyMoveSpeed);
          this.dangerState = true;
        } else {
        ...
        this.physics.overlap(gameObj, this.flowerPhysicsGroup) ?
          ...
          gameObj.setDamping(false).setDrag(1).setMaxVelocity(this.scene_13_settings.enemyMoveSpeed);
      }
    });

    // health check
    if (this.healthVal > 0) {
      this.healthVal = gameFunctions.control(gameState, this.scene_13_settings, this.healthVal);
