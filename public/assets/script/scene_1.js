...
class Scene_1 extends Phaser.Scene {
  ...
  create() {
    ...
    // img background
    this.add.image(0, 0, 'bg').setOrigin(0, 0).setDepth(this.scene_1_settings.backgroundDepth);

    // camera department
    this.camera = this.cameras.main.setBounds(0, 0, this.scene_1_settings.worldWidth, this.scene_1_settings.worldHeight);
    this.physics.world.setBounds(0, 0, this.scene_1_settings.worldWidth, this.scene_1_settings.worldHeight);

    // level title
    this.levelText = this.add.text(
      this.scene_1_settings.canvasWidth / 2,
      this.scene_1_settings.canvasHeight / 2,
      'Level 1',
      { fontSize: 30, color: '#000000' }
    ).setOrigin(0.5).setScrollFactor(0).setDepth(this.scene_1_settings.messageDepth);

    // initialize health
    [this.bonusScore, this.coinCount, this.healthVal] = [0, 0, this.scene_1_settings.playerStartingHealth];
    gameState.healthBar = this.add.sprite(40, 20, 'health_100').setScrollFactor(0).setDepth(this.scene_1_settings.healthBarDepth);

    // initialize player & controls
    gameState.cursors = this.input.keyboard.createCursorKeys();
    gameState.player = this.physics.add.sprite(
      this.scene_1_settings.playerSpawnPosition[0] * 32,
      this.scene_1_settings.playerSpawnPosition[1] * 32,
      'f_dog').setSize(30, 30).setDepth(this.scene_1_settings.playerSpriteDepth).setOrigin(0);
    ...
    // time tracker
    this.levelTime = this.scene_1_settings.levelTime;
    this.add.rectangle(this.scene_1_settings.canvasWidth / 2 + 70, 10, 260, 15).setFillStyle(0xffffff, 0.5).setScrollFactor(0).setDepth(this.scene_1_settings.scoreTimerBackgroundDepth);
    this.timeText = this.add.text(
      this.scene_1_settings.canvasWidth / 2, 10,
    ...
    this.scoreText = this.add.text(
      this.scene_1_settings.canvasWidth / 2 + 150, 10,
      `Score: ${this.score}`,
      { fontSize: 12, color: '#000000' }
    ).setOrigin(0.5).setScrollFactor(0).setDepth(this.scene_1_settings.scoreTextDepth);

    // scene transition 
    this.girl = this.physics.add.sprite(this.scene_1_settings.familySpawnPosition[0] * 32, this.scene_1_settings.familySpawnPosition[1] * 32, 'girl').setOrigin(0);
    ...
    gameState.emitter.once('death_bgm', () => {
      ...
      this.levelText = this.add.text(
        this.scene_1_settings.canvasWidth / 2,
        this.scene_1_settings.canvasHeight / 2 - 50,
        'Game Over',
        { fontSize: 30, color: 'white' }).setScrollFactor(0).setOrigin(0.5).setDepth(this.scene_1_settings.messageDepth);
      this.tweens.add({
        targets: this.levelText,
        alpha: 0,
        duration: 2000,
      ...
      const mask = this.add.rectangle(
        ...
        this.camera.displayHeight + 20).setFillStyle(0x000000, 0.2).setScrollFactor(0).setDepth(this.scene_1_settings.deathBackgroundMaskDepth);
      this.tweens.add({
        targets: mask,
        duration: 4000,
        fillAlpha: 0.9
      ...
      const unmask = this.add.circle(
        ...
        10
      ).setFillStyle(0xffffff, 0).setScrollFactor(0).setDepth(this.scene_1_settings.deathBackgroundUnmaskDepth);
      this.tweens.add({
        targets: unmask,
        duration: 4000,
        fillAlpha: 0.9
    ...
    const audioButton = this.add.sprite(
      this.scene_1_settings.canvasWidth - 20,
      this.scene_1_settings.canvasHeight - 20,
      'audio_button_on').setScale(0.5).setScrollFactor(0).setDepth(this.scene_1_settings.buttonDepth).setInteractive();
    ...
    this.backButton = this.add.sprite(
      this.scene_1_settings.canvasWidth / 2,
      this.scene_1_settings.canvasHeight / 2 + 50,
      'back_button').setDepth(this.scene_1_settings.buttonDepth).setVisible(false).setOrigin(0.5).setScrollFactor(0).setInteractive();
    ...
    // trees
    level_1.firTree.forEach(obj => { walls.create(obj.x * 32, obj.y * 32, 'fir_tree').setSize(12, 10).setOffset(25, 54).setDepth(this.scene_1_settings.treeSpriteDepth) });
    level_1.birchTree.forEach(obj => { walls.create(obj.x * 32, obj.y * 32, 'birch_tree').setSize(16, 12).setOffset(26, 46).setDepth(this.scene_1_settings.treeSpriteDepth) });

    // things
    level_1.bench.forEach(obj => { walls.create(obj.x * 32, obj.y * 32 - 16, 'bench') });
    level_1.lightPost.forEach(obj => { walls.create(obj.x * 32 - 16, obj.y * 32 - 8, 'light_post').setSize(14, 16).setOffset(10, 46).setDepth(this.scene_1_settings.wallSpriteDepth) });
    ...
    this.consumable.getChildren().forEach(gameObj => {
      this.physics.add.overlap(gameState.player, gameObj, () => {
        (this.healthVal + this.scene_1_settings.boneHealthRegen) > 100 ? this.healthVal = 100 : this.healthVal += this.scene_1_settings.boneHealthRegen;
    ...
    this.scene_1_settings.coins.forEach(coin => { this.collectables.create(coin.x * 32 - 16, coin.y * 32 - 16, 'coin').setCircle(15, 1, 0) });
    this.collectables.getChildren().forEach(gameObj => {
      this.physics.add.overlap(gameState.player, gameObj, () => {
        this.bonusScore += this.scene_1_settings.coinScoreBonus;
    ...
    this.physics.add.collider(this.moveable, gameState.player, () => {
      if (recycleCollider) {
        ...
        const message = this.add.text(
          this.scene_1_settings.canvasWidth / 2,
          this.scene_1_settings.canvasHeight - 40,
          str,
    ...
    enemiesPhysicsGroup.getChildren().forEach(function (gameObj) {
      ...
      if (gameObj.getData("tweenX") !== 0) {
        this.tweens.add({
          ...
          ease: 'Linear',
          duration: Math.abs(gameObj.getData("tweenX")) * this.scene_1_settings.enemyTweenDurationMultiplier,
          repeat: -1,
          yoyo: true,
          loopDelay: this.scene_1_settings.enemyTweenLoopDelay
        });
        ...
        this.tweens.add({
          ...
          ease: 'Linear',
          duration: Math.abs(gameObj.getData("tweenY")) * this.scene_1_settings.enemyTweenDurationMultiplier,
          repeat: -1,
          yoyo: true,
          loopDelay: this.scene_1_settings.enemyTweenLoopDelay
        });
  ...
  animate() {
    this.anims.create({
      ...
      frameRate: Math.round(this.scene_1_settings.moveSpeed / 15),
      repeat: -1
    });
    this.anims.create({
      key: 'b_move',
    ...
    this.anims.create({
      ...
      frameRate: Math.round(this.scene_1_settings.moveSpeed / 40),
      repeat: -1
    });
  }
  ...
  update() {
    ...
    if (gameState.player) {
      this.physics.overlap(gameState.player, this.semiWalls) ?
        ...
        gameState.player.setDamping(false).setDrag(1).setMaxVelocity(this.scene_1_settings.moveSpeed);
    }
    ...
    this.tweens.getAllTweens().forEach(tween => {
      ...
      if (gameObj.type === "Sprite") {
        ...
        if (this.physics.overlap(gameState.player, gameObj)) {
          ...
          if (this.healthVal > 0) {
            this.healthVal -= this.scene_1_settings.enemyHealthReduction;
            this.dangerState = true;
          } else {
            this.HealthVal = -1;
          }
        } else if (distanceCalc(gameState.player, gameObj) < this.scene_1_settings.enemyChaseDistance) {
          tween.pause();
          this.physics.moveToObject(gameObj, gameState.player, this.scene_1_settings.enemyMoveSpeed);
          this.dangerState = true;
        } else {
        ...
        this.physics.overlap(gameObj, this.semiWalls) ?
          ...
          gameObj.setDamping(false).setDrag(1).setMaxVelocity(this.scene_1_settings.enemyMoveSpeed);
      }
    });

    // health check
    if (this.healthVal > 0) {
      this.healthVal = gameFunctions.control(gameState, this.scene_1_settings, this.healthVal);
